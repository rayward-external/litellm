...
async def _normalize_claude_model(
    ...
    if request is not None:
        request.scope[_CLAUDE_MODEL_NORMALIZED] = (
            True  # rebind-ok: marks this request as already normalized so retries don't re-run the routing lookup
        )
    if source is None:
        return
    request_data["model"] = (
        source  # rebind-ok: rewrites the request body to the resolved model group before it reaches the router
    )
