...
def _redact_standard_logging_object(
    payload: Mapping[str, object],
) -> dict[str, object]:  # mutable-ok: upstream code, byte-identical to BerriAI/litellm's litellm_internal_staging
...
def perform_redaction(model_call_details: dict, result, redact_streaming_responses: bool = True):
    ...
    if isinstance(params, dict) and isinstance(request, Mapping):
        model_call_details["litellm_params"] = {  # rebind-ok: upstream code, byte-identical to upstream
            **params,
    ...
    if isinstance(standard_logging_object, Mapping):
        model_call_details["standard_logging_object"] = _redact_standard_logging_object(  # rebind-ok: upstream code
