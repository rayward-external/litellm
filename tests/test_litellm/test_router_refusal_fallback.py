...
is set, `_should_raise_content_policy_error` treats a matching refusal like a
content-policy violation so content_policy_fallbacks can route it elsewhere.
...
def test_refusal_matcher_matches_refusal_ignores_normal(monkeypatch):
    monkeypatch.setenv(
        "LITELLM_REFUSAL_FALLBACK_PATTERNS", r"i'?m sorry.*cannot assist,i cannot help with"
    )
...
def test_should_raise_on_refusal_only_when_enabled(monkeypatch):
    ...
    monkeypatch.setenv("LITELLM_REFUSAL_FALLBACK_PATTERNS", r"i'?m sorry.*cannot assist")
